pipeline {
    ...
    stages {
        stage('Checkout Code') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'jshema334', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_TOKEN')]) {
                    git url: "https://$GIT_USER:$GIT_TOKEN@github.com/Hema334/docker.git", branch: 'main'
                }
            }
        }
        ...
        stage('Login to Docker Registry') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'jshema334', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
